...
Player.prototype.add_point = function() {
  ...
  var new_p = { x: p.x, y: p.y };
  this.path.push(new_p);
  var inversed = (this.direction % 4) == 0 || (this.direction % 4) == 3; // the 2 movements where movement is inversed (-1)
  this.path_listener && this.path_listener(inversed ? new_p : p, inversed ? p : new_p, this.direction % 2 == 1);
}
...
Player.prototype.update_movement = function(start_time, end_time) {
  while(this.queue.length > 0 && this.queue[0].time <= end_time) {
    ...
    if(this.hittest_callback && this.hittest_callback())
      return;
...
Player.prototype.update = function(start_time, end_time) {
  this.update_movement(start_time, end_time);
};
